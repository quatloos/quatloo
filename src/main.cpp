...
// Copyright (c) 2014 The Quatloo developers
...
#if defined(NDEBUG)
# error "Quatloo cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xbcb2bb00da6bdf5f80e2afce42307b7fe664f3ec789b6c61936e03ea0e9cf5cc");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Quatloo: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Quatloo Signed Message:\n";
...
static const int64 nOriginalInterval = 160; //160 blocks = 6 hrs
//static const int64 nTargetTimespan = 6 * 60 * 60; // Litecoin: 3.5 days ; Quatloo: 6 hours
...
/*unsigned int static GetNextWorkRequired_KGW(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
	static const int64	BlocksTargetSpacing			= 2.25 * 60; // 2.25 minutes
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x94a28d0a7ecf0f184eef21f8639ade99866f059e72bf9bfe0fe5491c424a9f1b");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Reuters 6/14/2014  Iraq insurgent advance slows, U.S. sends carrier to Gulf";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1403726808;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086464999;

        if (fTestNet)
        {
            block.nTime    = 1403726320;
            block.nNonce   = 266341;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x4564403b2d130131b228445025c86eecfc78b40c0c7853b7d71eef081e72443e"));
...
unsigned char pchMessageStart[4] = { 0xfa, 0xce, 0xbe, 0xda }; // Litecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("QuatlooMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("QuatlooMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("QuatlooMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("quatloo-miner");
    ...
    try { loop {
        ...
        printf("Running QuatlooMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("QuatlooMiner terminated\n");
        throw;
    }
}
